...
class Snake(Env):
    ...
    def _GetOBS(self):
        # returns list of numpy arrays, one for each player
        return [player.getOBS(self.snake_players, self.apple_positions) for player in self.snake_players]
    ...
    def _GetRandomApplePosition(self):
        ...
        choices = [
            choice for choice in self.whole_coord if choice not in all_snake_pos and choice not in self.apple_positions
        ]
    ...
    def _GetRenderImg(self, renderer=100):
        ...
        for color, player in zip(colors_list, self.snake_players):
            ...
            cv2.rectangle(
                img=img,
                pt1=(head[0] * renderer, (self.size - head[1]) * renderer - renderer),
                pt2=(head[0] * renderer + renderer, (self.size - head[1]) * renderer),
                color=color[0],
                thickness=-1,
            )
            ...
            for position in player.getPosition()[1:-1]:
                cv2.rectangle(
                    img=img,
                    pt1=(position[0] * renderer, (self.size - position[1]) * renderer - renderer),
                    pt2=(position[0] * renderer + renderer, (self.size - position[1]) * renderer),
                    color=color[1],
                    thickness=-1,
                )
            ...
            cv2.rectangle(
                img=img,
                pt1=(tail[0] * renderer, (self.size - tail[1]) * renderer - renderer),
                pt2=(tail[0] * renderer + renderer, (self.size - tail[1]) * renderer),
                color=color[2],
                thickness=-1,
            )
        ...
        for apple_position in self.apple_positions:
            cv2.rectangle(
                img=img,
                pt1=(apple_position[0] * renderer, (self.size - apple_position[1]) * renderer - renderer),
                pt2=(apple_position[0] * renderer + renderer, (self.size - apple_position[1]) * renderer),
                color=self.colors["red"][1],
                thickness=-1,
            )
